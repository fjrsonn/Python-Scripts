...
def main():
    ...
    while not escolha:
        ...
        # Terceiro Nivel - SQLmap
            ...
            while True:
                argumento = input("SQLMAP: ").strip()
                if os.path.isfile("sqlmap.py"):
                    os.system("clear")
                    executar_script("sqlmap.py", argumento)
                else:
                    print("sqlmap.py não encontrado!")

                voltar = input("'v' para voltar ao menu ou 'ENTER' para continuar com SQLMAP: ")
                if voltar.lower() == 'v':
                    break

        # Quarto Nivel - Sherlock
        elif nivel == 4:
            while True:
        ...
        # Quinto Nivel - MrHolmes
        elif nivel == 5:
            while True:
                argumento = input("MR.HOLMES: ").strip()
                if os.path.isfile("$HOME/Mr.Holmes/MrHolmes.py"):
                    executar_script("$HOME/Mr.Holmes/MrHolmes.py", argumento)
                else:
                    print("MrHolmes.py não encontrado!")

                continuar = input("'v' para voltar ao menu ou 'ENTER' para continuar com MR.HOLMES: ")
                if continuar.lower() == 'v':
                    break

        # Sexto Nivel - Scan Subdomínios
        elif nivel == 6:
            while True:
        ...
        # Setimo Nivel - Scan Hosts Proxys
        elif nivel == 7:
            while True:
        ...
        # Oitavo Nivel - Execução de outro script
        elif nivel == 8:
            while True:
